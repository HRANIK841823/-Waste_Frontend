...
export default function ProductDetails() {
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      console.log("API.getUsers response status:", res.status); // If using Axios
      console.log("API.getUsers raw response data:", res.data); // If using Axios
      // Check if the response itself is an array OR if res.data is an array
      const userData = Array.isArray(res) ? res : (res && Array.isArray(res.data) ? res.data : []);
       
      setAllUsers(userData as User[]);
      return userData as User[];
    } catch (e) {
      // Improved error logging for debugging API.getUsers
      console.error("Failed to fetch all users. Check API.getUsers() implementation or backend endpoint:", e);
      setAllUsers([]);
      return [];
    }
  };

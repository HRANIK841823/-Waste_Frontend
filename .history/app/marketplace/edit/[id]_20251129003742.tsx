...
// --- FIX APPLIED HERE ---
// Changed 'import API from "../api"' to 'import * as API from "../api"'
// This ensures that all exports (default or named) from the API module are accessible 
// directly via API.functionName, resolving the module resolution conflict.
...
export default function EditWasteItemScreen() {
    ...
    const { data: productData, isLoading, isError, error, refetch } = useQuery<FetchedProduct>({
        ...
        queryFn: async () => {
            ...
            // FIX: Changed API.getItem to API.getItemDetail, matching the function name in your api.js file.
    ...
    // 2. Derive initial values from fetched data. 
    const initialValues: PostValues = {
        name: productData?.name || '',
        title: productData?.title || '',
        description: productData?.description || '',
        status: productData?.status || 'available',
        // Convert price number to string for the TextInput
    ...
    const handleUpdate = async (values: PostValues) => {
        ...
        try {
            ...
            // Call the update API endpoint (API.updateItem is correct)
    ...
    if (isError || !productData) { 
        // Display a more helpful error message in the UI
        let errorMessage = 'An unknown error occurred.';
        if (!itemId) {
            errorMessage = 'Invalid Item ID provided in the URL.';
        } else if (error) {
            // Check if error is an object with a message property
            const message = error.message || String(error);
            errorMessage = message.includes('404') ? 'Item not found (404). Check if the ID is correct.' : `Error: ${message}`;
        }
    ...
    return (
        ...
        <View style={styles.container}>
            <Text style={styles.header}>Edit Waste Item ✏️ (ID: {itemId})</Text>
            <Formik
                // Use initialValues derived from the successfully loaded productData
                initialValues={initialValues as PostValues}
                validationSchema={PostSchema}
                onSubmit={handleUpdate}
                // enableReinitialize is still needed to update the form fields when productData loads

...
export default function ProductDetails() {
  ...
  // We only store the ID of the current user
  ...
  /**
   * REPLACED fetchCurrentUserProfile. 
   * Finds the user ID from the logged-in user's profile API call.
   * This is still necessary to know *who* the current user is.
   */
  const fetchCurrentUserId = async () => {
    try {
      // Assuming getProfile returns { data: { id: '...' } }
  ...
  /**
   * Function to fetch all users. (No change needed here)
   */
   ...
   * Now uses currentUserId instead of currentUser object.
   */
...
    let isContactBuyer = false; 
    ...
    // Cases 1, 2, 3 handle pending/sold items or the owner's available items (unchanged)
    // ...

    // Case 4: General Customer/Buyer viewing an available item
    // If the user is NOT the owner and the item is available, show the seller's contact.
    // This is the case for any anonymous or logged-in non-owner viewing the listing.
    if (!isOwner && loadedProduct.status === 'available') { // 👈 The critical addition
        contactUserId = loadedProduct.seller_id;
        isContactBuyer = false; // Contact is the Seller
    }
    
    // Fallback: If the item is owned, and status is available, show owner/seller contact
    else if (isOwner && loadedProduct.status === 'available') {
        contactUserId = loadedProduct.seller_id;
        isContactBuyer = false;
    }
    // ... (rest of the logic for pending/sold transactions)

    if (contactUserId) {
      // Look up user in the pre-fetched list
...
  };
  ...
  const fetchProductAndUser = async (productId: string) => {
    ...
    const [productRes, userId, allUsersData] = await Promise.all([
      ...
      fetchCurrentUserId(), // Fetch the minimal required user data (ID)
      fetchAllUsers(),
    ]);
  ...
  useEffect(() => {
    ...
    // Only proceed if we know the user's ID and have all the user data
    if (product && currentUserId && allUsers.length > 0) {
  ...
  const handleBuy = async () => {
    ...
    if (!currentUserId) { // Check for ID instead of object
  ...
  // [Rest of the component's rendering logic, ContactCard, and Styles remain the same]
  // ... (omitted for brevity, assume the rendering code below is identical)
  if (!product) {
    return (
      <View style={styles.center}>

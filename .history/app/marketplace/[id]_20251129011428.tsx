...
import { SafeAreaView } from 'react-native-safe-area-context'; // Use SafeAreaView for better edge handling
import API from "../api";
...
const BORDER_LIGHT = '#E9ECEF'; // Slightly darker border for contrast
...
  email: string;
  name: string;
  // Added contact_number to User interface, as suggested to pull from /auth/me
...
      if (currentUser?.id === sellerId) {
            // Use contact info from currentUser profile (auth/me)
          setSellerContact({
...
        
      // Fetch seller contact if needed immediately
...
  // Re-fetch seller contact if currentUser changes after product load
  useEffect(() => {
...
      
      fetchSellerContact(product.seller_id); // Fetch contact after success
      
    } catch (err) {
...
          <View style={[styles.divider, { marginBottom: 100 }]} />
          
        </View>
      </ScrollView>
      
      {/* --- FIXED BOTTOM ACTION BAR (NEW for phone view professionalism) --- */}
...
  scrollContent: { 
    paddingBottom: 100, // Reserve space for the fixed action bar
},
...
    paddingBottom: 25, // For better iOS safe area spacing
    paddingTop: 15,
  },

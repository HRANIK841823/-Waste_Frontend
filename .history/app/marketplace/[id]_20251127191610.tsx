...
// Interfaces for better type safety (Keep these for TypeScript projects)
interface Product {
  _id: string;
  title: string;
  price: number | string;
...
      alert(`You successfully requested to buy ${product.title}! The seller will be notified.`);
...
      
      // Use the specific error message in the alert
      alert("Failed to initiate purchase: " + specificError); 
    }
  };
  
  // New handler for owner's edit button
  const handleEdit = () => {
      if (id) {
  ...
  // OWNERSHIP CHECK: Compare logged-in user ID with product's seller_id
    ...
    <ScrollView style={styles.container}>
      ...
      {!isProductOwner && (
        <TouchableOpacity
          ...
          disabled={isSold || product.status === 'pending'} // Also disable if pending purchase
        >
          <LinearGradient
            colors={isSold || product.status === 'pending' ? ["#ccc", "#aaa"] : ["#00E676", "#00BFA5"]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={styles.buyButton}
          >
            <Text style={styles.buyButtonText}>
              {isSold ? 'Item Sold' : product.status === 'pending' ? 'Pending Purchase' : 'Buy Now'}
            </Text>
          </LinearGradient>
        </TouchableOpacity>
      )}

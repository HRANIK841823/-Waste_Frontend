...
// 1. Define a type for the User (assuming structure)
interface User {
  id: string; // The unique ID of the logged-in user
  // other user properties...
}

// 2. Define a type for the Product (ensure owner_id is present)
interface Product {
  id: string;
  title: string;
  price: number | string;
...
  owner_id: string; // Crucial property for ownership check
}

export default function ProductDetails() {
  const params = useLocalSearchParams();
  const id = params.id;
  // Use the new Product type
  const [product, setProduct] = useState<Product | null>(null);
  const [loading, setLoading] = useState(true);
    
  // 3. State to hold the logged-in user's data
  // In a real app, this would come from a global state/context (like Redux, Zustand, or AuthContext)
...
        // 4. Fetch the current user data (Mocked for demonstration)
        fetchCurrentUser(); 
    } else {
        setLoading(false);
    }
...
  // 5. Mock function to fetch current user (Replace with actual Auth method)
  const fetchCurrentUser = () => {
    // !!! IMPORTANT: Replace this with your actual authentication logic 
    // to get the ID of the currently logged-in user.
    // Example: A user ID 'user-123'
    setCurrentUser({ id: 'user-123' }); 
      
    // In a real app, this might look like:
    // const user = AuthContext.useUser(); 
    // setCurrentUser(user);
  };
...
        
      // !!! ASSUME res.data HAS 'owner_id' 
      // For the product data to be useful, it MUST include the owner's ID.
      // Example structure: { id: 'p-456', title: '...', owner_id: 'user-456' }
      setProduct(res.data);
    } catch (err) {
...
  }
  
  // 6. Logic to check for ownership
  const isProductOwner = currentUser && product.owner_id === currentUser.id;
  
  // Determine if the Buy button should be disabled (already sold)
...
      )}
      
      {isProductOwner && ( // Display a badge for the owner
        <Text style={[styles.soldBadge, { backgroundColor: '#333' }]}>YOUR PRODUCT</Text>
      )}
...
      {/* 7. Conditional Rendering: Only show the button if NOT the product owner */}
      {!isProductOwner && (
        <TouchableOpacity 
          activeOpacity={0.8} 
          onPress={handleBuy} 
...
      {/* If the owner views it, they could see an 'Edit' button instead */}
      {isProductOwner && (
        <TouchableOpacity 
          activeOpacity={0.8} 
          onPress={() => alert('Navigate to Edit Product Screen')} 
          style={styles.buyButtonWrapper} // Reuse wrapper style
        >
          <LinearGradient
            colors={["#3F51B5", "#283593"]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={styles.buyButton} // Reuse button style
          >
            <Text style={styles.buyButtonText}>

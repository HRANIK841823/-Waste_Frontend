...
export default function ProductDetails() {
   ...
   * Logs are REMOVED as testing confirmed this part works.
   */
  const fetchAllUsers = async () => {
    try {
      const res = await API.getUsers(); 
      
      // Robustly handles if the API returns the array directly or nested under 'data'
   ...
   * This is where the fetched 'allUsers' array is used.
   */
  const setRelevantContact = (
    ...
    if (isOwner && loadedProduct.buyer_id && loadedProduct.status !== 'available') {
      ...
      isContactBuyer = true; // The contact we want to display is the buyer
    } 
    // Case 2: Buyer viewing an item they bought/requested (pending/sold)
    else if (currentUserId === safeToString(loadedProduct.buyer_id) && loadedProduct.status !== 'available') {
      contactUserId = loadedProduct.seller_id;
      isContactBuyer = false; // The contact we want to display is the seller
    }
  ...
  useEffect(() => {
    ...
    // This handles scenarios like when handleBuy updates the product state
  ...
  const handleBuy = async () => {
    ...
    try {
      // API call should update status to 'pending' and set buyer_id
      ...
      // Note: This relies on the current state of 'allUsers' which was fetched initially.
  ...
  // Determine if the contact card should be visible

...
        
        // Ensure data is an array before setting state, falling back to empty array
        if (Array.isArray(res.data)) {
          setProducts(res.data);
        } else {
          setProducts([]);
          console.warn("Marketplace API did not return an array.");
        }
        
        // 🎯 CRITICAL DEBUG STEP 🎯
        console.log("--- Marketplace Data Received ---");
        // Log the IDs of the first 5 products to verify they exist and are strings
        (Array.isArray(res.data) ? res.data : []).slice(0, 5).forEach((p, index) => {
            console.log(`Product ${index + 1} ID: ${p.id}, Type: ${typeof p.id}`);
        });
        console.log("---------------------------------");
        
    } catch (err) {
...
        // Set products to empty array on error to prevent filter crash
        setProducts([]); 
    } finally {
        setIsLoading(false);
    }

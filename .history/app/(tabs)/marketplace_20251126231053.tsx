...
import API from "../api"; // adjust path if required
...
export default function Marketplace() {
  ...
  // Keep the same categories UI; backend expects string exact match
  const categories = [
  ...
  const fetchMarketplace = async () => {
    try {
      ...
      // Only allow status filter as required: available or sold. For marketplace default show available only.
      ...
      console.log("Fetch marketplace error:", err.response?.data || err.message || err);
    }
  };
  ...
  const renderProduct = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/marketplace/${item.id || item._id}`)}
    >
      <Image source={{ uri: item.image_url || "https://via.placeholder.com/300" }} style={styles.image} />
      ...
      <Text
        style={[
          styles.price,
          (item.price === 0 || item.price === "Free") && { color: "#F6A700", fontWeight: "700" },
        ]}
      >
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Bottom Navbar (kept placeholder in case you use it) */}
    </View>
  );
}

// Keep your original styles exactly unchanged

import { Picker } from '@react-native-picker/picker'; // You may need to install this package
...
interface PostValues {
  ...
  price: string; // Stored as string, converted to number on submission
  location: string;
...
export default function PostWasteItemScreen() {
  ...
  const pickImage = async (setFieldValue: (field: string, value: any, shouldValidate?: boolean) => void) => {
    // Check for permissions if on a physical device
    if (Platform.OS !== 'web') {
  ...
  // --- Submission Handler ---
  const handleSubmit = (values: PostValues) => {
    setLoading(true);
    // 💡 Replace with your actual API call to upload data

...
export default function Marketplace() {
  ...
  const fetchMarketplace = async () => {
    try {
      ...
      params.status = "available"; // show only available items
  ...
  const renderProduct = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/marketplace/${item._id || item.id}`)}
    >
      ...
      <Text style={[styles.price, item.price === 0 && { color: "#F6A700", fontWeight: "700" }]}>
        ...
        <View style={styles.searchContainer}>
          ...
          {searchQuery.length > 0 && (
            <TouchableOpacity onPress={() => setSearchQuery("")}>
              <Ionicons name="close-circle" size={20} color="#999" style={{ marginHorizontal: 8 }} />
            </TouchableOpacity>
          )}
        </View>
      </View>

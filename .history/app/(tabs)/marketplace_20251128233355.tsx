...
  // 💡 FIX: Define fetchProducts inside the component using useCallback
  const fetchProducts = useCallback(async () => {
    setIsLoading(true);
    try {
        // 💡 FIX: Define params here where selectedCategory is in scope
        const params = selectedCategory === "All" ? {} : { category: selectedCategory }; 

        const res = await API.getMarketplace(params);
        
        let productsArray: Product[] = []; // Explicitly type the local array

        if (Array.isArray(res.data)) {
            productsArray = res.data; // Direct array
        } else if (res.data && Array.isArray(res.data.results)) {
            productsArray = res.data.results; // Extracted from 'results' key (DRF style)
        } else {
...
        // Log the IDs of the first 5 products to verify they exist and are strings
        productsArray.slice(0, 5).forEach((p, index) => {
            console.log(`Product ${index + 1} ID: ${p.id}, Title: ${p.title}`);
        });
        console.log("---------------------------------");
        
    } catch (err) {
...
        setIsLoading(false);
    }
  }, [selectedCategory]); // 💡 Dependency: Recreate fetchProducts when category changes

  // 💡 FIX: Run fetchProducts when the component mounts AND when the category changes
  useEffect(() => {
    fetchProducts();
  }, [fetchProducts]); // Depend on fetchProducts (which depends on selectedCategory)


  // This part was already correct for handling non-array state, but we keep the structure.
...
    // NOTE: Since fetchProducts now filters by category on the backend, 
    // this category filter is technically redundant but harmless if the API query is correct.
...
  }
  ...
  // Display Empty State if no products found after loading
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            <View style={[styles.container, { alignItems: 'center', paddingTop: 50 }]}>
                ...
                <TouchableOpacity onPress={() => setSelectedCategory("All")} style={{ marginTop: 20, padding: 10, backgroundColor: '#00C853', borderRadius: 8 }}>
